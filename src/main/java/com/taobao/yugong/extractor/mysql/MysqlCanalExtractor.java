  ...
  private List<IncrementRecord> fetchCanalRecord(JdbcTemplate jdbcTemplate,
    ...
    List<IncrementRecord> records = message.getEntries().stream()
        ...
        .map(entity -> {
          ...
          try {
            rowChange = CanalEntry.RowChange.parseFrom(entity.getStoreValue());
          } catch (InvalidProtocolBufferException e) {
            logger.error("parse entity failed, entity offset: {}", entity.getHeader().getLogfileOffset());
            throw new YuGongException("parse entity failed, stopped");
          }
          List<CanalEntry.RowData> rows = rowChange.getRowDatasList();
          // one entity contains multi row changes
